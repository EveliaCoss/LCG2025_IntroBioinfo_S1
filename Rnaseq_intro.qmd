...
[![Visualización de la estructura de un archivo FASTQ y la interpretación de la escala de calidad Phred para nucleótidos. Se clasifican las bases según su precisión en tres categorías: baja calidad (Q \< 20), calidad media (20 ≤ Q \< 30) y alta calidad (Q ≥ 30).](img/Calidad_Phred_33.png)](https://es.wikipedia.org/wiki/Formato_FASTQ#:~:text=El%20formato%20FASTQ%20es%20un,%C3%BAnico%20car%C3%A1cter%20del%20c%C3%B3digo%20ASCII.)

::: callout-note
## Ejercicio 3

1.  Visualiza el contenido del archivo "\_files/secuencias_bash.fastq" empleando head.

    ``` {.bash code-copy="true" eval="false"}
    head _files/secuencias_bash.fastq
    ```

2.  Calcula el numero de lineas contenidas en este archivo.

    ``` {.bash code-copy="true" eval="false"}
    wc -l _files/secuencias_bash.fastq
    ```

3.  Imprime las lineas 4 a 8 del archivo.

    ``` {.bash code-copy="true" eval="false"}
       sed -n '4,8p' _files/secuencias_bash.fastq
    ```

4.  Podemos utilizar la función `grep -c` para contar el número de secuencias de un archivo (de nuevo, sustituya archivo por el nombre de uno de los archivos):

    ``` {.bash code-copy="true" eval="false"}
       grep -c "^@SRR" _files/secuencias_bash.fastq
    ```
:::
...
-   [Wikipedia: Formato FASTQ](https://es.wikipedia.org/wiki/Formato_FASTQ#:~:text=El%20formato%20FASTQ%20es%20un,%C3%BAnico%20car%C3%A1cter%20del%20c%C3%B3digo%20ASCII.)
